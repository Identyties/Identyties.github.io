...
/* Having the mute button can help to make the visuals alone the sole focus for those who want it and is a generally good ease of access tool */
...
/* I figured that due to the trippy nature of the video and the fact I want to lean into that, having the ability to loop would be really good to keep that feeling sustained. */

function loopingTime (){
  if (videoElement.loop){
    //loop if active 
   videoElement.loop = false
   // I tried adding  loopButton.style.backgroundColor = "var(--col--03)"; and other variations to make the active button remain colourful to indicate it is on
   // But it didn't work despite my trying multiple things so I added css of :focus to the button to make it work, however that will change if either of the other two buttons are pressed 
   // and the loop would still be active, so my problem is not solved but it is at least semi-functional.
   // I looked into naming the on and off states, so then I could specifically target the on state to have a coloured background to indicate its on but couldn't figure it out.
  } else {
   // dont loop if not
   videoElement.loop = true
  }
}
loopButton.addEventListener("click", loopingTime);
